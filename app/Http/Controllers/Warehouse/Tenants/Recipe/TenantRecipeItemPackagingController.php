...
class TenantRecipeItemPackagingController extends Controller
{
    ...
    public function index()
    {
        ...
        $packagings = TenantRecipeItemPackagings::on('tenant')
            // ->with(['contacts', 'addresses']) // assuming these are the relations
            ->latest()
            ->paginate(10); // 15 is the number of items per page


        return view('warehouse.tenants.inventory.recipes.item.index', ['packagings' => $packagings, 'warehouse' => $this->warehouse]);
    }
    ...
    public function create()
    {
        ...
        return view("warehouse.tenants.inventory.recipes.item.create", ['warehouse' => $this->warehouse]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $packaging = TenantRecipeItemPackagings::on('tenant')->create($packaging);

            dispatch(new WriteAuditLogJob('created', 'Created Packaging \'' . $packaging->name . '\'', $this->user->id, $this->warehouse));

            DB::commit();
            return redirect()->route('warehouse.tenants.inventory.recipes.item.index')->with('success', "Successfully created a new Packaging");
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            $packaging = TenantRecipeItemPackagings::on('tenant')->find($id);
            ...
            DB::commit();
            return redirect()->route('warehouse.tenants.inventory.recipes.item.index')->with('success', "Successfully removed the Packaging from this warehouse.");
        } catch (\Exception $e) {
            DB::rollBack();
