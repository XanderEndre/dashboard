<x-app-layout>
    <x-layout.breadcrumbs :links="['Inventory' => route('warehouse.tenants.inventory.index'), 'View' => '#']" pageTitle="Vendors" />

    <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 lg:gap-8 mb-5">
        <!-- Card -->
        <div class="col-span-4 md:col-span-1">
            <x-cards.card>
                <x-cards.header :title="$item->name" :description="'Created By: '" />
                <x-cards.body>
                    @if ($imageUrl)
                        <img src="{{ $imageUrl }}" alt="Inventory Image">
                    @else
